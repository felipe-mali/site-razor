...
const IMAGES_PATH = path.resolve(__dirname, process.env.IMAGES_PATH || 'rede');
const USUARIOS_PATH = path.resolve(__dirname, process.env.USUARIOS_PATH || path.join('data', 'usuarios.json'));
...
// ═══════════════════════════════════════════
const FOTOS_CLIENTES_PATH = path.resolve(__dirname, process.env.FOTOS_CLIENTES_PATH || path.join('rede', 'fotos_clientes'));
...
app.use((erro, req, res, next) => {
  ...
  res.status(500).json({
    error: falhaDeGravacao
      ? 'O servidor não conseguiu gravar na pasta funcionarios/data. Verifique as permissões da pasta.'
      : 'Erro interno ao processar a solicitação.'
  });
});

app.listen(PORT, () => {
  console.log(`Sistema de funcionários iniciado na porta ${PORT}`);
});

...
async function carregarUsuarios() {
  ...
  const usuariosOrdenados = Object.entries(usuarios).sort(([, a], [, b]) =>
    String(a.nome || '').localeCompare(String(b.nome || ''), 'pt-BR', { sensitivity: 'base' })
  );

  tbody.innerHTML = usuariosOrdenados.map(([key, u]) => `
    <tr>
      <td>${key}</td>
      <td>${u.nome}</td>
...
function abrirModal(id = null) {
  ...
  const msgErro = document.getElementById('usuario-msg-erro');
  msgErro.textContent = '';
...
document.getElementById('usuario-form').addEventListener('submit', async (e) => {
  ...
  if (!editId && !senha) {
    msgErro.textContent = 'Informe uma senha para o novo funcionário.';
    msgErro.style.display = '';
    return;
  }

  try {
    const response = await fetch(editId ? `/api/usuarios/${editId}` : '/api/usuarios', {
    ...
    if (!response.ok) throw new Error(data.error || 'Não foi possível salvar o funcionário.');
    fecharModal();
    await carregarUsuarios();
  } catch (erro) {
    msgErro.textContent = erro.message || 'Erro de comunicação ao salvar.';
    msgErro.style.display = '';
  }
});
